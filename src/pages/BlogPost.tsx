...
function BlogPost() {
  ...
  useEffect(() => {
    const loadPost = async () => {
      ...
      try {
        // Load metadata
        const metadataModule = await import(`/public/data/blog-page/${date}/metadata.yaml?raw`);
        const metadata = yaml.load(metadataModule.default as string) as {
          post: { title: string; date: string; author: string }
        };

        // Load post content
        const contentModule = await import(`/public/data/blog-page/${date}/post.md?raw`);
        const content = contentModule.default as string;

        // Load background (optional)
        let backgroundUrl = '';
        try {
          const bgModule = await import(`/public/data/blog-page/${date}/background.jpg?url`);
          backgroundUrl = bgModule.default as string;
        } catch {
          // Use blog page background as fallback
          try {
            const blogBgModule = await import('/public/data/blog-page/background.jpg?url');
              ...
              components={{
                ...
                img: ({ src, alt, ...props }) => {
                  ...
                  if (src && src.startsWith('./imgs/')) {
                    imageSrc = `/public/data/blog-page/${date}/imgs/${src.substring(7)}`;
                  }
